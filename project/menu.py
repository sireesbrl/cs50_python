...
def level_info(levels, choice) -> str:
    return f"{levels[choice-1]['level']}:\n{levels[choice-1]['description']}"
    ...
    try:
        ...
        match choice:
            case 1:
                designs.clr_screen()
                designs.get_header()
                print(level_info(levels, choice))
                go_back_to_levels()
...
def profession_info(professions, choice) -> str:
    return f"{professions[choice-1]['profession']}:\n{professions[choice-1]['description']}"
    ...
    try:
        ...
        match choice:
            case 1:
                designs.clr_screen()
                designs.get_header()
                print(profession_info(professions, choice))
...
def read_disciples() -> None:
    ...
    if permission not in ["M", "V", "P"]:
        ...
        options_available(permission)
    try:
        table: list = Disciples.file_reader()
...
def check_edit_disciples(name, disciple_type, level, profession, department, cultivation_law) -> bool:
    table: list = Disciples.check_file_reader()
    success = False
    for dict in table:
        if dict.get("Name") == name:
            dict["Disciple_type"] = disciple_type
            dict["Level"] = level
            dict["Profession"] = profession
            dict["Department"] = department
            dict["Cultivation_law"] = cultivation_law
            success = True
            break
    if success:
        with open("test_disciples.csv", "w") as file:
    ...
    try:
        ...
        if is_empty():
            ...
            while True:
                ...
                valid: bool = check_input(entry[1], entry[2], entry[3], entry[4])
                if not valid:
                    print("Invalid! Going back to Main...")
                    designs.delay(2)
                    options_available(permission)
                    break
                for dict in table:
...
def check_delete_disciples(name) -> bool:
    table = Disciples.check_file_reader()
    new_table = []
    success = False
    for dict in table:
        if dict.get("Name") == name:
